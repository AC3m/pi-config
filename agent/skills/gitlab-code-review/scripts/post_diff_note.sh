...
if [[ -z "$new_line" && -z "$old_line" ]]; then
  echo "Either --line or --old-line is required." >&2; exit 1
fi

: "${old_path:=$new_path}"

if [[ -z "$token" ]]; then
  token="$(glab auth status --show-token 2>/dev/null | sed -n 's/.*Token found: //p' | head -n1)"
fi

if [[ -z "$token" ]]; then
  echo "No GitLab token found. Set GITLAB_TOKEN or authenticate with glab." >&2; exit 1
fi
...
[[ -n "$new_line" ]] && curl_args+=(--form "position[new_line]=$new_line")
[[ -n "$old_line" ]] && curl_args+=(--form "position[old_line]=$old_line")
